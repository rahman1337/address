#!/usr/bin/env python3
# brute2.py
# Simplified iSH-friendly bruteforce scanner
# Blockchain.info balance only, dictionary.txt -> found.txt

from __future__ import annotations
import io, sys, time, binascii, hashlib, logging
import requests
...
class Wallet:
    ...
    def _priv_to_compressed_pubkey(self, priv_hex):
        ...
        px = vk.to_string(); x=px[:32]; y=px[32:]
...
class BlockchainInfo:
    def __init__(self):
        self.base_url="https://blockchain.info/q/addressbalance"
        self.session=requests.Session()
        self.session.headers.update({"User-Agent":"brute2/1.0"})

    def get_balance(self, address):
        try:
            r=self.session.get(f"{self.base_url}/{address}",timeout=15)
            r.raise_for_status()
...
def main():
    ...
    sleep_sec=1.0
    addr_types=['p2wpkh','p2sh-p2wpkh','p2pkh','p2tr']
