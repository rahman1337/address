#!/usr/bin/env python3
import os, sys, time, signal, secrets, hashlib, random, threading
        ...
        for backoff in self.backoffs:
            ...
            try:
                ...
                if resp.status_code==200:
                    time.sleep(self.delay)  # no jitter
    ...
    def btc_balance(self, addr):
        ...
        for backoff in self.backoffs:
            ...
            try:
                resp=self.session.get(url,timeout=self.timeout)
                if resp.status_code==200:
                    time.sleep(self.delay + random.uniform(0,0.4))  # jitter
                    data=resp.json()
...
def read_word_file_2048(file_path):
    ...
    if len(words)!=2048: raise RuntimeError(f"Wordlist must be 2048 words")
    return words
...
def bip39_seed_from_mnemonic(mnemonic):
    return hashlib.pbkdf2_hmac("sha512",mnemonic.encode(),"mnemonic".encode(),2048,dklen=64)

def derive_priv_for_path_from_seed(seed,purpose,coin,account=0,change=0,index=0):
    root=bip32utils.BIP32Key.fromEntropy(seed)
    node=root.ChildKey(purpose+bip32utils.BIP32_HARDEN).ChildKey(coin+bip32utils.BIP32_HARDEN).ChildKey(account+bip32utils.BIP32_HARDEN).ChildKey(change).ChildKey(index)
...
# ----------------- Scanning -----------------
def scan_address(priv_bytes, addr_type, checker, mnemonic):
    ...
    if recvd>0:
        ...
        with print_lock:
            ...
            print("WIF:", privkey_to_wif(priv_bytes))
            print("Address type:", addr_type)
            print("ADDRESS:", addr)
            print("RECEIVED (BTC):", recvd/1e8)
            print("BALANCE (BTC):", (bal/1e8 if bal is not None else "NONE"))
            print("="*53+"\n")

# ----------------- HD generator -----------------
def hd_unique_mnemonic_generator(wordlist_path, seen_path):
    ...
    while not stop_event.is_set():
        ...
        privs={}
        skip=False
        for purpose,keyname in [(44,"p2pkh"),(49,"p2sh"),(84,"p2wpkh")]:
            try:
                priv=derive_priv_for_path_from_seed(seed,purpose,0,account=0,change=0,index=0)
                priv_int=int.from_bytes(priv,"big")
                if priv_int==0 or priv_int>=SECP256K1_ORDER: skip=True; break
                privs[keyname]=priv
            except: skip=True; break
        if skip: continue
        seen.add(mnemonic)
        persist_seen(seen_path,mnemonic)
        yield mnemonic, privs
...
def main():
    ...
    checker=AddrChecker(delay=0.6)
    gen=hd_unique_mnemonic_generator(wordlist_file, seen_file)
    print(f"[INFO] Using HD mode (wordlist {wordlist_file}), persisting seen mnemonics -> {seen_file}")

    with ThreadPoolExecutor(max_workers=3) as executor:
        futures=set()
        try:
            for mnemonic, privs in gen:
                if stop_event.is_set(): break
                for keytype in ["p2pkh","p2sh","p2wpkh"]:
                    fut=executor.submit(scan_address, privs[keytype], keytype, checker, mnemonic)
                    futures.add(fut)

                # process completed
                done, _ = as_completed(futures, timeout=0), None
                for d in list(done):
                    futures.discard(d)
                    try: d.result()
                    except Exception as e:
                        with print_lock: print("[ERROR]", e)
