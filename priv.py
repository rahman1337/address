...
def sha256(b): h=SHA256.new(); h.update(b); return h.digest()
def ripemd160(b): h=RIPEMD160.new(); h.update(b); return h.digest()
...
def base58_encode(b):
    ...
    chars=[]
    while num>0: num,rem=divmod(num,58); chars.append(BASE58_ALPHABET[rem])
    return "1"*zeros+"".join(reversed(chars)) if chars else "1"*zeros
def base58check_encode(payload): chk=sha256(sha256(payload))[:4]; return base58_encode(payload+chk)
def privkey_to_wif(priv_bytes,compressed=False):
    prefix=b'\x80'; payload=prefix+priv_bytes
    if compressed: payload+=b'\x01'
    return base58check_encode(payload)

# ---- bech32 helpers ----
BECH32_CHARSET="qpzry9x8gf2tvdw0s3jn54khce6mua7l"
def bech32_polymod(values): GENERATORS=[0x3b6a57b2,0x26508e6d,0x1ea119fa,0x3d4233dd,0x2a1462b3];chk=1
    for v in values: b=(chk>>25)&0xFF; chk=((chk&0x1FFFFFF)<<5)^v
        for i in range(5): 
            if (b>>i)&1: chk^=GENERATORS[i]
    return chk
def bech32_hrp_expand(hrp): return [ord(x)>>5 for x in hrp]+[0]+[ord(x)&31 for x in hrp]
def bech32_create_checksum(hrp,data): values=bech32_hrp_expand(hrp)+list(data)+[0,0,0,0,0,0]; polymod=bech32_polymod(values)^1; return bytes([(polymod>>(5*(5-i)))&31 for i in range(6)])
def bech32_encode(hrp,data): combined=bytes(list(data)+list(bech32_create_checksum(hrp,data))); return hrp+'1'+''.join([BECH32_CHARSET[b] for b in combined])
def convertbits(data,frombits,tobits,pad=True):
    ...
    elif not pad and (bits>=frombits or ((acc<<(tobits-bits))&maxv)): raise ValueError
    return bytes(ret)
def bech32_p2wpkh_from_h160(h160): return bech32_encode('bc',bytes([0])+convertbits(h160,8,5))
...
class IndexProvider:
    ...
    def next_index(self):
        with self._lock: val=self._i; self._i+=1
...
def worker(name,idx_provider,checker,print_lock,debug):
    ...
    while not stop_event.is_set():
        ...
        try:
            pk=PrivateKey(priv_bytes); pubc=pk.public_key.format(compressed=True)
            ...
            with print_lock: print(f"[ERROR] [{name}] idx={idx} failed: {e}",flush=True)
            time.sleep(0.6)
    if debug:
...
def main():
    ...
    def _signal_handler(sig,frame):
        stop_event.set()
        with last_priv_lock: hex_checkpoint=last_priv_hex
        if hex_checkpoint:
