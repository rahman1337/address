#!/usr/bin/env python3
"""
6-chain mnemonic scanner with automatic BTC wallet path discovery
Chains: Ethereum, BSC, Polygon, Solana, Sui, Bitcoin
- 3 accounts per mnemonic
- Predictable-ish mnemonics (repetitive/sequential/mixed)
- MNEMONIC + WIF + PRIVATE_KEY + ADDRESS + BALANCE prints
- Error logging in normal mode
- Retry/backoff for RPC (1s, 2s, 3s)
- Uses only public nodes
"""
import os, sys, time, threading, asyncio, struct, traceback, random
...
repetitive_index = 0
mixed_index = 0
MIXED_PATTERNS = WORDLIST[:32]  # first 32 words for predictable-ish

def gen_mnemonic():
    global sequential_counter,repetitive_index,mixed_index
    mode = GENERATOR_MODES[random.randint(0,2)]
    words = []
    for _ in range(12):
        if mode=='repetitive':
            words.append(WORDLIST[repetitive_index%len(WORDLIST)])
            repetitive_index+=1
        elif mode=='sequential':
            words.append(WORDLIST[sequential_counter%len(WORDLIST)])
            sequential_counter+=1
        else:
            words.append(MIXED_PATTERNS[mixed_index%len(MIXED_PATTERNS)])
            mixed_index+=1
    return ' '.join(words)
...
async def rpc_post_with_retries(url: str, json_payload: dict, session: aiohttp.ClientSession, debug: bool=False, max_attempts: int=3):
    ...
    for attempt in range(1,max_attempts+1):
        try:
            ...
            if attempt<max_attempts: await asyncio.sleep(backoff[attempt-1])
    raise last_exc

# Ethereum-like get balance
...
async def sui_get_balance(rpc_url,address,session,debug=False):
    ...
    j=await rpc_post_with_retries(rpc_url,payload,session,debug)
    if "error" in j: raise RuntimeError(f"RPC error: {j['error']}")
    val=j.get("result",{}).get("totalBalance",0)
    return int(val)

# Bitcoin with auto wallet detection
async def btc_get_balance(base_rpc:str,address:str,session:aiohttp.ClientSession,debug=False):
    payload={"jsonrpc":"2.0","id":1,"method":"getreceivedbyaddress","params":[address,0]}
    try:
        j=await rpc_post_with_retries(base_rpc,payload,session,debug)
    except Exception as e: raise
    if "error" in j:
        err=j["error"]
        code=err.get("code") if isinstance(err,dict) else None
        msg=err.get("message") if isinstance(err,dict) else str(err)
        if code==-19 or "multiple wallets" in str(msg).lower():
            # try auto listwallets
            candidates=[]
            try:
                j2=await rpc_post_with_retries(base_rpc,{"jsonrpc":"2.0","id":1,"method":"listwallets","params":[]},session,debug)
                if isinstance(j2,dict) and "result" in j2 and isinstance(j2["result"],list):
                    candidates=j2["result"]
            except: pass
            last_exc=None
            for wn in candidates:
                try:
                    url=base_rpc.rstrip("/")+"/wallet/"+wn
                    pl={"jsonrpc":"2.0","id":1,"method":"getreceivedbyaddress","params":[address,0]}
                    rj=await rpc_post_with_retries(url,pl,session,debug)
                    if "error" in rj: raise RuntimeError(rj["error"])
                    return int(rj.get("result",0))
                except Exception as e:
                    last_exc=e
            raise last_exc or RuntimeError(err)
                    ...
                    for priv,_ in derive_secp256k1_bip44(seed,coin_map[chain_name],indices=(0,1,2)):
                        ...
                        if chain_name=="bitcoin":
                            ...
                            for addr_type,addr in addrs.items():
                                ...
                                try: bal_sats=await btc_get_balance(BTC_RPC,addr,session,debug)
                                except Exception as e:
                            ...
                            try: bal=await eth_like_get_balance(rpc_url,eth_address_from_priv(priv),session,debug)
                            except Exception as e: print(f"[error][{chain_name}] {e}"); continue
                            if bal>0:
                                bal_str=f"{bal/1e18:.18f} ETH (wei={bal})"
                                print(format_found_block(chain_name,mnemonic,privhex,eth_address_from_priv(priv),bal_str))
                                append_found(f"{chain_name} | {mnemonic} | {privhex} | {eth_address_from_priv(priv)} | {bal_str}")
